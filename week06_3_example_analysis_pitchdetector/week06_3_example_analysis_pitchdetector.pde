//week06-3_example_analysis_pitchdetector
//想樣把聲音的最重要頻率(Hz)找出來，使用pitchdetector物件
/**
...
SoundFile sound;//新加這一行，要把beat.mp3讀近來
...
void setup() {
  ...
  // capture audio input
  //pitchDetector.input(new AudioIn(this, 0));//原本程式，是用audio in 音效卡的音源輸入
}
...
void draw() {
  ...
  if (pitchAndConfidence[1] > 0) {
    ...
    println(pitchAndConfidence[0], pitchAndConfidence[1]);//印出pitch跟confidence都印出來
  }
